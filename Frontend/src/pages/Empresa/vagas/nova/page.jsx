...
export default function NovaVaga() {
  ...
  const validateForm = () => {
    ...
    if (!formData.email_contato.trim()) newErrors.email_contato = "Email de contato é obrigatório"
    if (!formData.link_candidatura.trim()) newErrors.link_candidatura = "Link de candidatura é obrigatório"
    if (formData.vagasDisponiveis < 1) newErrors.vagasDisponiveis = "Deve haver pelo menos 1 vaga disponível"
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">{formData.titulo || "Título da Vaga"}</h1>
              <div className="flex flex-wrap gap-2 mb-4">
                <span className="px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm font-medium">
                  {formData.modalidade || "Modalidade"}
                </span>
                <span className="px-3 py-1 bg-orange-100 text-orange-800 rounded-full text-sm font-medium">
                  {formData.tipoContrato || "Tipo de Contrato"}
                </span>
                <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                  {formData.nivel || "Nível"}
                </span>
              </div>
              <p className="text-gray-600">{formData.descricao || "Descrição da vaga..."}</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-6">
                ...
                {formData.diferenciais.some((dif) => dif.trim()) && (
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-3">Diferenciais</h3>
                    <ul className="space-y-2">
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <p className="text-gray-600">{formData.salario || "A combinar"}</p>
                    </div>
                    <div>
                      <span className="font-medium text-gray-700">Localização:</span>
                      <p className="text-gray-600">{formData.localizacao || "Não informado"}</p>
                    </div>
                    <div>
                      <span className="font-medium text-gray-700">Vagas Disponíveis:</span>
                      <p className="text-gray-600">{formData.vagasDisponiveis}</p>
                    </div>
                    <div>
                      <span className="font-medium text-gray-700">Contato:</span>
                      <p className="text-gray-600">{formData.email_contato || "contato@empresa.com"}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Preencha as informações para criar uma nova vaga</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                ...
                />
                {errors.localizacao && <p className="mt-1 text-sm text-red-600">{errors.localizacao}</p>}
              </div>
              ...
              <div>
                ...
                {errors.tipoContrato && <p className="mt-1 text-sm text-red-600">{errors.tipoContrato}</p>}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange("vagasDisponiveis", Number.parseInt(e.target.value) || 1)}
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent ${
                    errors.vagasDisponiveis ? "border-red-300" : "border-gray-300"
                  }`}
                />
                {errors.vagasDisponiveis && <p className="mt-1 text-sm text-red-600">{errors.vagasDisponiveis}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            {errors.requisitos && <p className="mt-2 text-sm text-red-600">{errors.requisitos}</p>}
          </div>

          {/* Diferenciais */}
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-gray-900">Diferenciais</h2>
              <button
                type="button"
              ...
              {formData.diferenciais.map((diferencial, index) => (
                <div key={index} className="flex items-center space-x-3">
                  <input
                    type="text"
                    value={diferencial}
                    onChange={(e) => handleArrayChange("diferenciais", index, e.target.value)}
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-gray-900">Benefícios *</h2>
              <button
                type="button"
            ...
            {errors.beneficios && <p className="mt-2 text-sm text-red-600">{errors.beneficios}</p>}
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                />
                {errors.email_contato && <p className="mt-1 text-sm text-red-600">{errors.email_contato}</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Link de Candidatura *</label>
                <input
                  type="url"
                  value={formData.link_candidatura}
                  onChange={(e) => handleInputChange("link_candidatura", e.target.value)}
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent ${
                    errors.link_candidatura ? "border-red-300" : "border-gray-300"
                  }`}
                  placeholder="https://empresa.com/candidatura"
                />
                {errors.link_candidatura && <p className="mt-1 text-sm text-red-600">{errors.link_candidatura}</p>}
              </div>
            </div>
          </div>
          ...
          {/* Status */}
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">Status da Vaga</h2>
